(() => {
  ...
  style.textContent = `
    .active-exercise{overflow:hidden}
    .active-exercise.is-collapsed .active-exercise-body{display:none}
    .active-exercise.is-complete{border-color:rgba(216,255,62,.36)}
    .exercise-head{cursor:pointer}
    .exercise-head-actions{display:flex;gap:7px;align-items:center}
    .exercise-order{display:flex;gap:5px}
    .exercise-order button,.exercise-toggle{width:36px;height:36px;border-radius:10px;border:1px solid var(--line);background:var(--panel2);color:var(--text);padding:0}
    .exercise-order button:disabled{opacity:.28}
    .exercise-toggle{color:var(--accent);font-weight:900}
    .collapsed-summary{display:none;padding:12px 14px;border-top:1px solid var(--line);background:var(--panel2);grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center}
    .active-exercise.is-collapsed .collapsed-summary{display:grid}
    .collapsed-summary strong{display:block}
    .collapsed-summary small{color:var(--muted)}
    .collapsed-summary span{color:var(--accent);font-size:.78rem;font-weight:800;text-align:right}
    @media(max-width:560px){
      .exercise-head-actions{align-items:flex-end;flex-direction:column}
      .exercise-order button,.exercise-toggle{width:34px;height:34px}
      .collapsed-summary{grid-template-columns:1fr}
      .collapsed-summary span{text-align:left}
    }
  `;
    ...
    box.innerHTML = active.exercises.map((exercise, exerciseIndex) => {
      ...
      const sets = exercise.sets.map((set, setIndex) => `
        <div class="set-line ${set.completed ? 'completed' : ''}">
          ...
          <button type="button" class="set-done" data-complete-set="1" data-ei="${exerciseIndex}" data-si="${setIndex}">✓</button>
        </div>
      `).join('');
        ...
        <article class="active-exercise ${collapsed ? 'is-collapsed' : ''} ${summary.complete ? 'is-complete' : ''}">
          <div class="exercise-head" data-exercise-head="${exerciseIndex}">
            ...
            <div class="exercise-head-actions">
              ...
              <button type="button" class="remove-exercise" data-remove-exercise="${exerciseIndex}">✕</button>
            </div>
          </div>
