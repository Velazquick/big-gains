...
### Program Foundation v1 design boundary

The proposed [Program Foundation v1 specification](PROGRAM_FOUNDATION_V1.md) inserts an explicit multi-session organization layer between Goals and Train without implementing it. Goal remains the portable exact-exercise destination; Program owns a split-agnostic ordered sequence of slots, optional preferred calendar anchors, block/review policy, Goal links, authority state, and pinned immutable Routine versions; Routine owns one reusable ordered exercise/set/rep/rest prescription; Train materializes an independent active-workout snapshot; History retains performed facts; and Progress/Insights derive interpretations.
...
Programming authority is explicit: `off` grants no apply authority; `review` permits explainable proposals that require user approval and is the v1 maximum; `auto` is reserved for a later optional trust phase with a narrow allowlist, reason trace, undo/review, and absolute protection for identity, active sessions, and completed history. Applying any accepted proposal remains a separate stale-base-guarded transaction with a future effective boundary and rollback through another new version. Program Foundation adds no runtime behavior, persistence, schema, Supabase/RLS, sync, or production-data change.
