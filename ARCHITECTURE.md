...
Program Analyzer = deterministic structural facts, not coaching/recommendation. `program-setup.js` recomputes the result in memory from the selected local Program version, profile-owned Routine versions, the static EKF catalog, linked Goal references, and matching explicit `sequenceState`. It persists no analysis cache, recommendation, proposal, or derived authority. The contained read-only UI opens an existing linked Goal card without letting Goals mutate Program. Schema v5, Supabase, cloud shadows, Train, active workouts, completed History, and Progress evidence remain unchanged.
...
Release `v89-program-1b-deterministic-analyzer` adds `program-analyzer.js` and the read-only analyzer surface to the revisioned offline shell while retaining the v86 startup authority gate, schema-v5 local Program capture, and the same cloud-config content revision. Release `v87-program-1a-canonical-routine-capture` introduced `program-model.js`, `program-setup.js`, and `program-setup.css`.
