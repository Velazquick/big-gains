...
`asset-manifest.js` is the single asset inventory. Release `v75-progress-v2-history-explorer-detail` retains the v72 bodyweight effective-load behavior, v73 guarded cross-device adoption, and v74 completed-workout management while cache-busting the new archive/detail presentation. Persisted set load remains added load; analytics receives the latest valid profile bodyweight as live context and derives effective load without rewriting history. The manifest applies the release query parameter to every production asset, rejects duplicate core assets, and supplies the same immutable manifest to both page loaders and the service worker. `index.html`, `auth-setup.html`, both loaders, the manifest, service-worker core, web manifest, icon, local chime, and all revisioned CSS and scripts form the precached app shell.
