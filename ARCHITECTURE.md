...
| `program-domain-envelope.js` | `BigGainsProgramDomainEnvelope` | Dormant PPS1 Slice 2A pure projection boundary. It reads a normalized schema-v5 `programCapture` without mutation; emits the exact `{}` no-Program payload or a contract-versioned definitions/heads/sequence envelope; validates scope, graph closure, pins, lineage, authority, sequence, revisions, and the member manifest; and produces strict canonical JSON plus aggregate/component/member SHA-256 fingerprints. It is intentionally absent from runtime assets and owns no DOM, storage, queue, transport, recovery, Auth, or cloud behavior. |
...
| `program-domain-recovery.js` | `BigGainsProgramDomainRecovery` | Runtime-unattached Slice 3 recovery boundary. It capability-gates a single RLS-owned row read, validates physical owner plus semantic client scope, classifies local/remote state without mutation, validates Program-before-active-session dependencies, and exposes only safe whole-graph adoption through an exact raw snapshot/write/readback/rollback port. Legacy publication, conflict rebasing, and cutover remain deferred. |
| `program-domain-cutover.js` | `BigGainsProgramDomainCutover` | Runtime-unattached pre-rollout cutover orchestrator. It exposes capability-gated inspect, revision-1 legacy/empty publication, deterministic conflict snapshots, stale-choice rejection, rollback-safe cloud adoption, and contract-limited device rebasing through the frozen Program queue and guarded Edge/readback transport. It fails closed on pending Program work, active/rest state, immutable disagreement, invalid lineage, and unprovable sequence transitions. |
...
The hosted Program portability Slice 1 schema defines a dormant `program_domains` destination: one guarded whole-graph envelope row per profile with stable client ID `program-domain`, aggregate/component revisions and fingerprints, accepted-base metadata, and forced RLS. Its privileged compare-and-set implementation lives in `private`, accepts caller identity only from the JWT-verifying `program-domain-write` Edge gateway, and grants no execution to `PUBLIC`, `anon`, or `authenticated`; ordinary authenticated readback remains RLS-protected. The schema and gateway migrations create no rows, and the hosted table remains empty. `programCapture` therefore remains local-only and excluded from the active cloud record set, checksums, recovery reconstruction, and outbound queue until the separately reviewed capability rollout. A guarded remote training-data fast-forward or explicit same-entity resolution continues carrying forward the validated local-only capture while replacing cloud-backed records, preventing unrelated Program loss without claiming Program parity. Existing custom routines, Goals, Train, and cloud rows are unchanged.

Program portability Slices 2A, 2B, 3, and cutover are deliberately dormant. Their modules are not loaded by `index.html`, are not precached by the service worker, and receive no production capability. Slice 3 distinguishes missing row, explicit empty envelope, exact parity, safe remote advancement, pending local advancement, divergence, invalid remote state, active/rest deferral, pending Program queue state, and meaningful legacy-unpublished state. Safe adoption replaces the canonical Program graph as a unit inside one schema-v5 profile document, preserves local-only application traces and all non-Program fields, verifies the canonical remote fingerprint after readback, and restores the exact prior raw document on failure. Cutover adds deliberate first publication and real conflict choices, but only behind its explicit capability constructor. It rechecks remote state immediately before mutation, carries a frozen decision snapshot, preserves unrelated queue work, and refuses cases the PPS1 transition/lineage contract cannot prove safe. The hosted `program_domains` source boundary exists with zero rows; production Program publication/recovery and every rollout control remain absent/off.
