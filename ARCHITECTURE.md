...
| `program-domain-sync.js` | `BigGainsProgramDomainSync` | Runtime-unattached, production-disabled Program transport. It freezes durable whole-envelope operations, uses only the guarded RPC, verifies exact RLS-owned readback before ACK, and keeps Program ordering isolated from the ordinary five-table coordinator. |
...
| `program-domain-cutover.js` | `BigGainsProgramDomainCutover` | Runtime-unattached pre-rollout cutover orchestrator. It exposes capability-gated inspect, revision-1 legacy/empty publication, deterministic conflict snapshots, stale-choice rejection, rollback-safe cloud adoption, and contract-limited device rebasing through the frozen Program queue and guarded RPC/readback transport. It fails closed on pending Program work, active/rest state, immutable disagreement, invalid lineage, and unprovable sequence transitions. |
...
RLS is enabled and forced on every table. The browser-facing `authenticated` role receives existing application-table operations, while `program_domains` grants direct `SELECT` only and routes writes through the fixed-search-path `put_program_domain_guarded` function. Profile-scoped access requires either account ownership or an exact `(auth.uid(), account_id, profile_id)` membership; the guarded function derives `account_id` from the profile mapping, takes a profile-scoped transaction lock, and rejects stale aggregate/component bases, downgrades, equal-revision disagreement, and non-idempotent retries. Members may read their containing account only for runtime resolution and cannot update it. Membership mutation remains administrative-only. Anonymous/public table grants are revoked. A profile UUID alone is insufficient because authorization always proves the account/profile relationship with a composite key.
