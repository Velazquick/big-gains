...
| `cloud-config.js` | `__BIG_GAINS_CLOUD_CONFIG__` | Empty/default-off checked-in browser configuration. The Pages workflow may replace it in the deployment artifact using browser-safe Actions variables for the project URL, publishable key, automatic-reconciliation gate, and exact version-1 Program-portability gate. Missing, false, malformed, or unexpected controls keep their capability off. |
...
| `program-domain-envelope.js` | `BigGainsProgramDomainEnvelope` | PPS1 pure projection boundary. It reads a normalized schema-v5 `programCapture` without mutation; emits the exact `{}` no-Program payload or a contract-versioned definitions/heads/sequence envelope; validates scope, graph closure, pins, lineage, authority, sequence, revisions, and the member manifest; and produces strict canonical JSON plus aggregate/component/member SHA-256 fingerprints. It is manifest-loaded for the rollout but owns no DOM, storage, queue, transport, recovery, Auth, or cloud behavior. |
...
| `program-domain-sync.js` | `BigGainsProgramDomainSync` | Capability-gated Program transport. It freezes durable whole-envelope operations, sends only the exact frozen fields through the authenticated `program-domain-write` Edge gateway, verifies exact RLS-owned readback before ACK, and keeps Program ordering isolated from the ordinary five-table coordinator. |
| `supabase/functions/program-domain-write` | authenticated Edge Function | JWT-required narrow Program write gateway. It verifies the caller with Auth, rejects non-exact or identity-bearing bodies, passes only parameterized frozen fields plus the server-derived caller UUID to the private database gate, and returns stable non-sensitive outcomes. |
| `program-domain-recovery.js` | `BigGainsProgramDomainRecovery` | Capability-gated whole-graph recovery boundary. It reads one RLS-owned row, validates physical owner plus semantic client scope, classifies local/remote state without mutation, validates Program-before-active-session dependencies, and adopts through exact raw snapshot/write/readback/rollback ports. |
| `program-domain-cutover.js` | `BigGainsProgramDomainCutover` | Explicit cutover orchestrator. It exposes capability-gated inspect, revision-1 legacy/empty publication, deterministic conflict snapshots, stale-choice rejection, rollback-safe cloud adoption, and contract-limited device rebasing through the frozen Program queue and guarded Edge/readback transport. It fails closed on pending Program work, active/rest state, immutable disagreement, invalid lineage, and unprovable sequence transitions. |
| `program-portability.js` | `BigGainsProgramPortability` | Optional runtime bridge for the exact version-1 deployment capability. It verifies Auth/account/profile mapping, owns a dedicated per-profile durable Program queue and accepted snapshot, derives only provable activation/successor/completion transitions, connects deliberate Plan actions to cutover/recovery, and exposes human-readable status while keeping revisions and fingerprints out of ordinary UI. Capability-off initialization performs no Program read, write, queue, or local mutation. |
...
| `measurement-units.js` | `BigGainsUnits` | The single profile-scoped weight conversion, parsing, input-step, and formatting boundary. Canonical storage and analytics stay in pounds; display/input may use pounds or kilograms. |
| `user-data-export.js` | `BigGainsUserDataExport` | Pure current-profile portability projection for completed-set CSV and curated versioned JSON, export-local relationship mapping, filename sanitization, native file-share feature detection, and Blob-download fallback. It owns no persistence, network, cloud, Auth, or import behavior. |
...
| `workout-session-controller.js` | `BigGainsWorkoutSessionController.create(...)`, pure factory helpers, and the `workoutSessionController` instance | Active-session ownership: manual and exact Program-derived start, resume/replace, routine loading and repair, exercise construction/addition/removal/order/focus, set creation/edit/adjust/complete, completion advancement, workout completion, and discard. It owns the existing set validation and mutation rules, signals TimerController through an injected port only after a qualifying completion has been saved, reads replaceable state/session values through live ports, and has no DOM, storage, cloud, or profile access. |
...
| `program-setup.js` | `BigGainsProgramSetup` | Owns the secondary Plan landing, Active Program detail, Today/Goal connective presentation, Program Analyzer rendering, Jorge's eight-stage Program review wizard, and the explicit next-Program-session action. It requires exact immutable Routine pins, delegates session materialization to `BigGainsProgramOrigin`, delegates Train mutation to `workoutSessionController`, and persists only through `app.js`'s schema-v5 gateway. |
...
This doctrine clarifies ownership across the accepted [Goals v1](GOALS_V1_SPEC.md), [Program Foundation v1](PROGRAM_FOUNDATION_V1.md), [Programming Engine v1](PROGRAMMING_ENGINE_V1.md), [Exercise Knowledge Foundation](EXERCISE_KNOWLEDGE_FOUNDATION.md), [Performance Records v1](PERFORMANCE_RECORDS_V1.md), and [synchronization](SYNC_SEMANTICS.md) contracts. It does not migrate code, rename runtime symbols, expand intelligence, or change schema v5, local-first behavior, Supabase, History, or production behavior.
...
4. DOM handlers in `app.js` translate set/exercise actions into `workoutSessionController` calls. The controller owns numeric/blank normalization, stepper adjustment, Add Set inheritance, exercise order/focus/collapse/removal, set removal, and completed toggles against the current live active object. Empty sets remove immediately; entered or completed working/warm-up rows require an inline second confirmation. Removal changes only the active workout array, persists before rendering, renumbers from current order, and never edits the Routine/Program prescription or `programOrigin`. Notes and per-exercise rest preferences remain with `workoutNotes`; their DOM adaptation remains in `app.js`. Rendering alone does not write storage.
...
6. `complete()` keeps only exercises with completed sets, tolerates an explicitly reduced set list, calculates duration, and derives both Performance Record families from authoritative History before prepending the completed workout exactly once. Rounded Epley v1 still passes only through the EKF eligibility/basis gate; machine-indicated `load_reps` work instead compares exact entered load and never becomes e1RM evidence. It then clears the active session/timer and calls the injected local persistence gateway before the completion-presentation callback. `app.js` creates its in-memory receipt from that saved workout and shows the focused completion screen. Done discards only the receipt and returns to Today; Review workout calls the existing history detail. Reloading cannot reconstruct the receipt or repeat the save. A workout with no completed sets cannot finish.
...
The normative [Performance Records v1 contract](PERFORMANCE_RECORDS_V1.md) is implemented as derived interpretation with exactly two families: **e1RM Record** and **Indicated Load Record**. `analytics.js` returns chronological source-set Record Events, per-workout event counts, and current exact-exercise state by family; the schema-v5 `prs` map remains the compatible latest-e1RM projection for existing consumers. History derives typed attribution directly, so `BEST` remains only the best eligible set inside that workout and never implies a historical Record Event. Machine-indicated presentation states its profile-local, exact-exercise, cross-machine limitation.
...
The [Product IA / Navigation Map v1](PRODUCT_IA_V1.md) defines the implemented persistent model: `Today / Plan / Train / Progress / Library`, with Goals and Program connected under Plan and completed-workout Calendar consolidated into Progress/History. Settings is a secondary route opened from the persistent profile header rather than a sixth primary destination. Goal, Program, Routine version, Workout/History, and derived Progress ownership remain unchanged.
...
The hosted `program_domains` schema defines one guarded whole-graph envelope row per deliberately cut-over profile with stable client ID `program-domain`, aggregate/component revisions and fingerprints, accepted-base metadata, and forced RLS. Its privileged compare-and-set implementation lives in `private`, accepts caller identity only from the JWT-verifying `program-domain-write` Edge gateway, and grants no execution to `PUBLIC`, `anon`, or `authenticated`; ordinary authenticated readback remains RLS-protected. The schema and gateway migrations create no rows. `programCapture` stays local-first and remains excluded from the ordinary five-table cloud record set; only the separately capability-gated Program bridge may project it into its dedicated envelope/queue. A guarded remote training-data fast-forward or explicit same-entity resolution continues carrying forward the validated local capture while replacing cloud-backed records. Existing custom routines, Goals, Train, and ordinary cloud rows are unchanged.

Program portability modules are revisioned, precached, and optional to core startup. The checked-in capability is OFF; only the generated exact `{ programPortability: true, programPortabilityVersion: 1 }` deployment pair activates the bridge. Recovery distinguishes missing row, exact parity, safe remote advancement, pending local advancement, divergence, invalid remote state, active/rest deferral, pending Program queue state, and meaningful legacy-unpublished state. Equality is established by rebuilding the local envelope at the accepted remote revisions and matching its aggregate and component fingerprints, not by comparing incidental immutable-catalog array order. Safe adoption replaces the canonical Program graph as a unit inside one schema-v5 profile document, preserves local-only application traces and all non-Program fields, verifies the canonical remote fingerprint after readback, and restores the exact prior raw document on failure. Cutover rechecks remote state immediately before mutation, carries a frozen decision snapshot, preserves unrelated queue work, and refuses cases the PPS1 transition/lineage contract cannot prove safe. The repository-controlled production deployment currently enables the exact version-1 pair after corrected physical publication/convergence and second-device adoption passed; remaining mutable-transition, successor, conflict, and offline physical acceptance is tracked separately from the default-off source boundary.
...
Each active exercise may carry `displayUnitOverride: "lb" | "kg"` as session-only UI metadata. Its effective unit is `exercise.displayUnitOverride ?? state.unitPreferences.weightUnit ?? "lb"`. No workout-wide override remains. On load, a valid legacy workout-wide override is copied to each current exercise lacking a valid exercise override, then deleted; invalid values are removed. This migration is idempotent and never touches canonical loads. New exercises and new workouts start from the live profile default. Existing active-session JSON preserves these additive fields locally/offline and through cloud shadow/recovery without a new protocol or schema. Completion strips exercise overrides before History/analytics, and History normalization strips any imported ones. Discard removes the session. Program/Routine prescriptions, `programOrigin`, History display preference, and canonical-pound exports remain unchanged.
...
- `bigGainsViewShell` owns Today/Plan/Goals/Train/Progress/Library presentation, the secondary Settings route, and safe legacy `history` and `calendar` route aliases. The persistent five are Today, Plan, Train, Progress, and Library; Settings opens from the profile header, Goals and Program select Plan as their owning destination, while History List, History Calendar, workout detail, and analytics select Progress. `workoutProgress` owns the inline History surface, always defaults a fresh History entry to List, and keeps view selection presentation-only. `BigGainsProgramSetup` owns origin-aware Plan ↔ Goal ↔ Program Back presentation. Finish leaves the completion screen in control; Done explicitly returns to Today.
...
Release `v91-plan-nav-history-list-calendar` promotes Plan into the primary five-item shell, moves Calendar into the inline Progress-owned History surface beside default List, removes the temporary Library Plan shortcut, and preserves legacy Calendar/History routes through aliases. It changes no asset inventory beyond revisioning existing shell files and keeps the same cloud-config content revision. Release `v90-program-setup-ux-v2-plan-bridge` introduced the substantive Plan landing and Active Program detail; release `v89-program-1b-deterministic-analyzer` introduced `program-analyzer.js`; release `v87-program-1a-canonical-routine-capture` introduced `program-model.js`, `program-setup.js`, and `program-setup.css`.

Candidate `v92-pe-1a-volume-neutral-exposure-redistribution` adds `programming-engine.js` and `programming-review.js`, revisions the Plan presentation assets, and updates the fail-closed offline manifest. It adds no persistence entity, schema, cloud/Supabase behavior, Program-driven Train path, completed-History mutation, release, or deployment.

Candidate `v93-pe-1b-program-origin-provenance` adds `program-origin.js` and one explicit Today/Plan action for the next active Program slot. The module validates and freezes `big-gains.program-origin.v1`, materializes the exact pinned Routine, advances the local rolling sequence only in the successful completion save, and derives completed cycles from complete compatible History slot coverage. `state-persistence.js` validates the optional object on active/completed workouts under schema v5. Existing generic `workouts` and `active_sessions` JSON shadow envelopes retain it without a new table, column, RLS policy, migration, or entity type. Manual, saved-routine, weekday, and retrospective entry do not receive origin; existing History is never backfilled.
...
History Calendar month/date selection lives only in `sessionStorage`, namespaced by profile. Completed workouts remain the sole source for both List and Calendar. Local `Date` components—not UTC string slicing—form grouping keys, so the calendar follows the browser's local timezone. Calendar workout buttons call the same history-detail renderer as List and add no new saved or synchronized fields.

Progress remains the History owner. Its compact preview opens an inline, non-modal History surface whose default List groups completed workouts newest-first by the browser's local month/year; the accessible `List | Calendar` switch changes only presentation. Calendar preserves month navigation, local-date selection, retrospective logging, and the shared completed-workout detail. Workout detail records its List or Calendar origin so Close returns to that view without nesting modal surfaces. Neither History view writes filters, view selection, analytics, or replacement workout payloads; only the existing explicit edit/delete workflows mutate a selected completed workout.

`retrospective-workout.js` owns one in-memory draft and never reads or writes `activeWorkout`, rest timers, Workout Mode, pet state, or timer feedback. Create mode parses the selected local date into local year/month/day components; optional time is applied with the local `Date` constructor and checked against the original local date key before save. Edit mode clones one exact completed workout into the same form, hides internal identity and ownership, and preserves its workout/exercise/set IDs, original timestamps, optional retrospective marker, optional Program origin, notes, and array order unless a visible field or ordering control changes them. Create mode never fabricates Program origin. Every performed set uses `workout-session-controller.js` completion validation, so bodyweight records store only added load while external-load sets require a positive weight. A failed persistence write restores the previous arrays/maps and leaves the draft open.

An edit replaces the matching workout array entry by stable ID and recomputes typed Record Events, per-workout counts, current record state, and the schema-v5 compatible e1RM `prs` projection from corrected History through `analytics.js`. Individual warm-up or working sets may be removed; entered/completed rows require inline confirmation, and the saved workout may contain fewer performed sets than its preserved `targetWorkingSets`. Deletion performs the same full-History recomputation after removing only the selected workout. The ordinary post-save semantic capture sees the stable workout fingerprint change as an upsert revision or its absence as a tombstone revision. Cloud failure remains asynchronous and cannot roll back a successful local edit/delete.
...
The separate user portability action passes only the loaded current profile object, matching local profile ID, human display/presentation values, local catalog, release marker, and one export timestamp into `BigGainsUserDataExport`. The module rejects a profile-ID mismatch and projects only explicit human-owned fields. It regenerates relationship references locally and never reads other profile keys, cloud storage, Auth state, queues, or transport envelopes. The CSV contains completed stored sets, including warm-ups; the curated JSON is intentionally not accepted by technical restore. Delivery stays client-side through a capability-checked two-file Web Share call or individual Blob downloads. See [User-owned data export v1](USER_DATA_EXPORT_V1.md).

Settings groups Profile, Appearance, Units, Training, Cloud & backup, and Support. [Appearance v1](APPEARANCE_V1.md) adds six profile-scoped accent choices through a versioned profile-row contract and guarded offline outbox. Legacy defaults stay unchanged until explicit selection; theme, pet behavior, and Performance/Soft styles are outside this feature. Normal Settings exposes only user-facing status and actions; raw queue state, revisions, fingerprints, recovery/reconciliation internals, profile identifiers, migration tooling, and the legacy snapshot gateway remain available inside the closed Support → Advanced diagnostics disclosure.
...
The SQL migrations define `accounts`, `profiles`, `profile_memberships`, `workouts`, `routines`, `bodyweight_entries`, `preferences`, `active_sessions`, `sync_metadata`, `tombstones`, and the dormant `program_domains` source boundary. All profile data carries the pair `(account_id, profile_id)`, and composite foreign keys prove that the profile belongs to that account. Ownership columns are immutable after creation. Completed workouts and bodyweight entries have unique account/profile/client IDs and account-scoped idempotency keys. Active sessions and Program domains are each unique per account/profile. `sync_metadata.metadata` stores only migration journal metadata. A Program-domain row stores one canonical JSON object plus aggregate/definitions/heads/sequence revision and fingerprint identities; `{}` with all component revisions at zero is the database-level explicit no-Program payload.

RLS is enabled and forced on every table. The browser-facing `authenticated` role receives existing application-table operations, while `program_domains` grants direct `SELECT` only. Program writes follow authenticated client → JWT-required `program-domain-write` Edge Function → `private.put_program_domain_guarded` → ordinary RLS readback. The Edge Function derives caller identity only from Auth verification and uses a parameterized server database connection; the private `SECURITY DEFINER` has an empty search path, is absent from the exposed API schema, and grants no execution to `PUBLIC`, `anon`, or `authenticated`. It derives `account_id` from the profile mapping, independently proves the server-passed caller UUID as account owner or exact managed member, takes a profile-scoped transaction lock, and rejects stale aggregate/component bases, downgrades, equal-revision disagreement, and non-idempotent retries. Direct authenticated insert/update/delete remains denied, and the service path receives no hard-delete privilege. Members may read their containing account only for runtime resolution and cannot update it. Membership mutation remains administrative-only. Anonymous/public table grants are revoked. A profile UUID alone is insufficient because authorization always proves the account/profile relationship with a composite key.
