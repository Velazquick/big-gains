...
- `bigGainsViewShell` owns Today/Plan/Goals/Train/Progress/Library presentation plus safe legacy `history` and `calendar` route aliases. The persistent five are Today, Plan, Train, Progress, and Library; Goals and Program select Plan as their owning destination, while History List, History Calendar, workout detail, and analytics select Progress. `workoutProgress` owns the inline History surface, always defaults a fresh History entry to List, and keeps view selection presentation-only. `BigGainsProgramSetup` owns origin-aware Plan ↔ Goal ↔ Program Back presentation. Finish leaves the completion screen in control; Done explicitly returns to Today.
