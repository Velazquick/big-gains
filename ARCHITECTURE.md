...
The proposed [Goals v1 specification](GOALS_V1_SPEC.md) defines a documentation-only, exercise-scoped strength-goal and progression-policy boundary. It is not implemented at this baseline. The existing profile-derived `goals` object and singleton cloud goals preference remain unchanged compatibility data; they do not currently authorize Train intervention or implement the proposed goal lifecycle.
...
The current five-destination shell remains unchanged. The design compares Goals entry from Today, a broader Plan destination, and a top-level utility launcher; a Today launch into a contained Goals hub is recommended for review, but the final navigation choice remains open. Goals remains destination-first (`Where am I going?`) while Progress remains evidence-first (`How am I doing?`).
