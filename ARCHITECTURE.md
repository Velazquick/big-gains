...
The checked-in Program portability Slice 1 migration defines a dormant `program_domains` destination: one guarded whole-graph envelope row per profile with stable client ID `program-domain`, aggregate/component revisions and fingerprints, accepted-base metadata, forced RLS, and an authenticated owner/member compare-and-set RPC. The migration creates no rows and has not been applied to the hosted project. `programCapture` therefore remains local-only and excluded from the active cloud record set, checksums, recovery reconstruction, and outbound queue until the separately reviewed hosted rollout. A guarded remote training-data fast-forward or explicit same-entity resolution continues carrying forward the validated local-only capture while replacing cloud-backed records, preventing unrelated Program loss without claiming Program parity. Existing custom routines, Goals, Train, and cloud rows are unchanged.
