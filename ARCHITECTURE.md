...
The deployed Jorge account is discovered as exactly two profiles with client IDs `jorge` and `alexa`. An exact `profile_memberships` row resolves a separate Auth user to one existing managed profile without changing the owner. A signed-in user with neither an account nor membership sees the Phase 4G independent onboarding action; its security-invoker RPC atomically creates one owned account and one server-issued independent profile. Any owner/member overlap, multiplicity, mismatch, or other account/profile shape blocks.
