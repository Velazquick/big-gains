...
The accepted [Exercise Knowledge Foundation contract](EXERCISE_KNOWLEDGE_FOUNDATION.md) defines canonical identity, measurement, taxonomy, provenance, and compatibility rules underneath this API. EKF-1 implements only stable identity and compatibility generation: 119 explicitly persisted opaque exercise IDs, 119 permanent legacy-ID mappings, two opaque family IDs, deterministic source validation, and the generated catalog. Deferred taxonomy, measurement, and analytics fields remain explicitly `unknown`/inactive in the EKF-1 source. Workout cards, schema-v5 records, analytics formulas/results, catalog contents, service-worker loading, and Supabase remain unchanged; those boundaries continue until separately approved later phases.
