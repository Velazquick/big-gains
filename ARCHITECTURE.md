...
- `bigGainsViewShell` owns the Today/Plan/Goals/Train/Calendar/Progress/Library view and session-scoped last-view memory. Plan and Goals are contextual destinations rather than sixth/seventh bottom-navigation items; the persistent five remain Today, Train, Calendar, Progress, and Library. `BigGainsProgramSetup` owns origin-aware Plan ↔ Goal ↔ Program Back presentation. Finish leaves the completion screen in control; Done explicitly returns to Today.
...
Release `v90-program-setup-ux-v2-plan-bridge` moves Program lifecycle presentation into a contextual Plan landing and Active Program detail, adds Today and Goal cross-links, expands Program Setup into a plain-language staged review, and retains Library only as a shortcut. It changes no asset inventory beyond revisioning the existing shell files and keeps the same cloud-config content revision. Release `v89-program-1b-deterministic-analyzer` introduced `program-analyzer.js`; release `v87-program-1a-canonical-routine-capture` introduced `program-model.js`, `program-setup.js`, and `program-setup.css`.
