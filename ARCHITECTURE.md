...
The normative [Performance Records v1 contract](PERFORMANCE_RECORDS_V1.md) makes PR an umbrella product term while preserving that current e1RM behavior exactly and defining a documentation-only Indicated Load Record for later implementation. The current `prs` map is latest per-exercise e1RM state and `workout.prs` is an e1RM event count; neither preserves complete historical event attribution. History's `BEST` chip selects the best e1RM-eligible working set within that workout and is not proof that the set established a historical Record Event.
