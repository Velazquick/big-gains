...
**RC2-6.12 — Semantic decomposition.** The later Program portability design MUST represent, at minimum, immutable Routine-version entities, immutable Program-version entities, mutable Program identity/head metadata, active/draft/archive pointers, mutable sequence state, and immutable approved application traces. Physical table layout MAY combine safely atomic records, but each logical identity and dependency MUST remain explicit and validated.
...
**RC2-6.17 — Local-first order.** Program create/edit/approve/activate, sequence advancement, and application-trace mutations MUST persist inside the canonical local schema-v5 profile before durable enqueue and MUST remain usable offline. Program portability does not make cloud the live editing authority.
...
**RC2-6.20 — Required dependent spec.** Runtime Program portability MUST NOT begin until a separately reviewed `PROGRAM_PORTABILITY_SYNC_V1.md` fixes physical tables/envelopes, client IDs, canonical fingerprints, graph-completeness boundary, dependency/transaction order, revision and tombstone rules, RLS/grants, local-to-cloud migration, queue/ACK behavior, recovery reconstruction, conflict UX, rollback, and existing-profile cutover. It MUST also define the backward-compatible transition from `big-gains.program-capture.v1` and its current `storageMode: local_only` marker without changing existing Program/Routine/version identities. This is an RC-2 dependency, not deferred product scope.
...
**RC2-14.2 — Program physical model.** The first-class semantic decomposition is fixed by RC2-6.11–RC2-6.20. Physical tables/envelopes, operation grouping, and cutover mechanics remain deliberately unresolved until the required Program portability sync spec proves their atomicity, RLS, revision, and recovery behavior.
