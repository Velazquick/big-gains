...
Release-review candidate `v92-pe-1a-volume-neutral-exposure-redistribution` adds the pure deterministic PE-1A engine and a contained Plan proposal display. After four exact comparable stalled exposures spanning at least two completed Program cycles and two post-Goals-adjustment opportunities, one active linked Goal exercise may receive a Review-only proposal from one to two exposures with identical total cycle working sets. Allocation is balanced with any remainder assigned to the earliest rolling Program position. A minimum auxiliary Routine variant is proposed only when a repeated pin requires it. Approval/application remains disabled for PE-1B; production History without explicit Program origin fails closed, and no Program/Routine/Goal/workout/History persistence, Train behavior, schema, Supabase, release, or deployment is changed.

The approved [Programming Engine v1 contract](PROGRAMMING_ENGINE_V1.md) defines the deterministic `inputs → explicit rules → proposal | no_change | unavailable → reason/evidence trace → faithful explanation` boundary for review. PE-1A now implements only volume-neutral `+1` exposure redistribution for one exact linked Goal exercise plus a necessary auxiliary Routine variant. Review remains the ceiling; Auto, `-1`, total-volume change, rep-range change, deloads, substitution, and free-form generation remain parked.
