...
Release `v75-progress-v2-history-explorer-detail` completes the first History V2 product slice. Progress keeps a compact three-session Recent Training preview, while View History opens a newest-first archive grouped by local month/year. The shared completed-workout detail now uses canonical duration, effective volume, exercise, working-set, PR, and bodyweight-load analytics in a responsive profile-themed surface; it does not add exercise click-throughs. The v74 Edit/Delete flows, IDs, payloads, schema-v5 storage, tombstones, queue revisions, and fast-forward behavior remain unchanged, with no parallel history store or persisted presentation state.
