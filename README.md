...
Release-review candidate `v95-mobile-startup-interactivity` adds a runtime-interactivity boundary after identity verification. The personalized shell remains inert and concealed until required controller/listener composition completes; required startup failures expose neutral recovery, while optional-module failures are recorded without disabling safe core navigation. The v94 PE-1C behavior remains unchanged: atomic local successor activation, immutable Program-origin sessions, and schema-v5 profile data retain their existing contracts. No proposal intelligence, History rewrite, schema bump, Supabase change, production mutation, merge, or deployment is included.
