...
Release-review candidate `v96-program-portability-rollout` attaches the previously dormant Program envelope, guarded transport, recovery, and cutover boundaries behind an exact version-1 deployment capability. Missing, false, malformed, or unknown capability values remain inert; ordinary cloud sync and local-first training continue independently. The runtime adds deliberate first publication, whole-graph adoption, explicit Program conflicts, durable offline Program operations, exact Edge/readback ACK, and human-readable Plan status without rewriting History or `programOrigin`. The hosted capability remains a separately staged deployment decision.
