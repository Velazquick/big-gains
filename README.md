...
Release `v90-program-setup-ux-v2-plan-bridge` adds a substantive secondary Plan destination without changing the five persistent tabs. Plan presents active Goals and the current or draft Program as connected peers, provides a contained Active Program detail with rolling-session and approved Routine drill-down, reuses Program-1B facts for highlights and full analysis, and supplies contextual Today and Goal ↔ Program traversal with origin-aware Back behavior. Program Setup is now an eight-stage plain-language review from source candidates through future-only activation; Library retains a labeled Plan shortcut rather than owning the Program lifecycle. Calendar, Train materialization/execution, active workouts, completed History, Progress, schema v5, Supabase/RLS/sync, and Programming Engine authority remain unchanged.
