...
for(const identity of ['signed-out','ordinary','independent','managed-member'])test(`${identity} direct Operator route fails closed`,async({page})=>{
  ...
  expect(await page.evaluate(()=>window.operatorRequests.filter(r=>r.name==='operator_query').length)).toBe(0);
});
...
for(const width of [1440,390])test(`owner Operator overview, pagination, detail and funnel at ${width}px`,async({page},info)=>{
  ...
  await page.getByRole('button',{name:'Users',exact:true}).click();await expect(page.locator('#content tbody tr')).toHaveCount(25);
  await page.locator('#search').fill('synthetic');await page.getByRole('button',{name:'Apply',exact:true}).click();
  ...
  await page.locator('.person').first().click();await expect(page.locator('#content')).toContainText('No private sets');
