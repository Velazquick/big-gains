...
export async function verifyOperatorV2({test,client,as,uuid}) {
 ...
 await check('v2 visitor path bounded, anonymous write-only, no Auth identity or campaign conversion',async()=>{
  ...
  const absent=await rpc({section:'traffic',source:'beta-wave-2'});assert.equal(absent.traffic.length,0);assert.equal(absent.visitor_windows.days7,0);assert.equal(absent.visitor_environments.length,0);
