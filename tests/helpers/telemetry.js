...
export function assertTelemetryRequest(request) {
  ...
  expect(Object.keys(event).sort()).toEqual(['id','event_name','profile_client_id','release','platform','browser','mode','surface','category','training_mode'].sort());
  expect(JSON.stringify(event).length).toBeLessThanOrEqual(1024);
  expect(event.id).toMatch(/^[0-9a-f-]{36}$/i);
  expect(['app_open','workout_started','workout_completed','program_adopted','exercise_swapped','stale_session_recovery_shown','stale_session_resumed','stale_session_finished','stale_session_discarded','recovery_required','conflict_presented','app_error']).toContain(event.event_name);
  expect(event.release).toBe('v114-operator-console-v1');
