...
const CURRENT_CACHE = 'big-gains-shell-v75-progress-v2-history-explorer-detail';
const PREVIOUS_CACHE = 'big-gains-shell-v74-history-v2-workout-management';
...
test('first install precaches one complete, revision-consistent app shell', async ({ page }) => {
  ...
  expect(state.release).toBe('v75-progress-v2-history-explorer-detail');
  expect(state.cacheNames).toContain(CURRENT_CACHE);
  expect(state.cachedUrls).toEqual(state.expectedUrls);
  expect(state.cachedUrls).toContain(new URL('/exercise-catalog.js?v=v75-progress-v2-history-explorer-detail', page.url()).href);
  expect(state.cachedUrls).toContain(new URL('/routine-engine.js?v=v75-progress-v2-history-explorer-detail', page.url()).href);
  expect(state.cachedUrls).toContain(new URL('/workout-session-controller.js?v=v75-progress-v2-history-explorer-detail', page.url()).href);
  expect(state.cachedUrls).toContain(new URL('/timer-controller.js?v=v75-progress-v2-history-explorer-detail', page.url()).href);
  expect(state.cachedUrls).toContain(new URL('/auth-setup.html', page.url()).href);
  expect(state.cachedUrls).toContain(new URL('/auth-setup.js?v=v75-progress-v2-history-explorer-detail', page.url()).href);
...
test('production assets use the manifest release once and contain no duplicate core entries', async ({ page, request }) => {
  ...
  expect(consistency.authSetupScripts).toEqual([
    './cloud-config.js?v=v75-progress-v2-history-explorer-detail',
    './vendor/supabase.js?v=v75-progress-v2-history-explorer-detail',
    './auth-setup.js?v=v75-progress-v2-history-explorer-detail'
  ]);
  expect(consistency.authSetupStyles).toEqual(['./auth-setup.css?v=v75-progress-v2-history-explorer-detail']);
