...
| **7. Programming Engine v1** | PE-1A pure proposal generation and Plan display implemented; atomic successor application remains deferred. One exact linked Goal exercise, `+1` exposure, identical cycle sets, Review only | Analyzer, exact Goal inputs, explicit Program-cycle provenance, Goals-adjustment evidence, versioned allocation policy |
...
Implementation tracking: Program-1B supplies the phase-5 deterministic calculation boundary. Release `v90-program-setup-ux-v2-plan-bridge` implemented phase 3 and contained phase-6 detail. Release `v91-plan-nav-history-list-calendar` completes the primary navigation and History migration. Candidate `v92-pe-1a-volume-neutral-exposure-redistribution` implements phase 7's first pure proposal slice and Plan review display. Candidate `v93-pe-1b-program-origin-provenance` implements the minimum phase-8 bridge: Today/Plan can explicitly start the active Program's exact next pinned Routine, and active/completed workouts retain immutable Program/Routine/slot/cycle origin. Rolling advancement occurs only on completion; full-cycle proof is derived from completed explicit origins. Manual and retrospective paths remain available without provenance, and old History is never guessed or backfilled. Approval remains parked for PE-1C atomic successor application. **Program Analyzer = deterministic structural facts, not coaching/recommendation.** Broader Train-selection migration, Programming Engine operations, and block-performance Progress remain deferred.
