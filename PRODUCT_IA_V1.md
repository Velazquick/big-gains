...
| **7. Programming Engine v1** | PE-1A pure proposal generation and Plan display plus PE-1C explicit atomic successor application implemented. One exact linked Goal exercise, `+1` exposure, identical cycle sets, Review only | Analyzer, exact Goal inputs, explicit Program-cycle provenance, Goals-adjustment evidence, versioned allocation policy |
| **8. Program-driven Train** | PE-1B minimal explicit next-session entry implemented; broader Train-selection migration remains deferred. Active Program supplies exact pinned materialization/provenance and the active snapshot remains stable | Program execution/provenance contract and canonical active Program; engine generation is not required |
...
Implementation tracking: Program-1B supplies the phase-5 deterministic calculation boundary. Release `v90-program-setup-ux-v2-plan-bridge` implemented phase 3 and contained phase-6 detail. Release `v91-plan-nav-history-list-calendar` completes the primary navigation and History migration. Candidate `v92-pe-1a-volume-neutral-exposure-redistribution` implements phase 7's first pure proposal slice and Plan review display. Candidate `v93-pe-1b-program-origin-provenance` implements the minimum phase-8 bridge: Today/Plan can explicitly start the active Program's exact next pinned Routine, and active/completed workouts retain immutable Program/Routine/slot/cycle origin. Candidate `v94-pe-1c-atomic-proposal-application` adds two-step inline approval, exact stale recomputation, atomic local successor activation, and a retained application trace. Manual and retrospective paths remain available without provenance, old History is never guessed or backfilled, and active workouts remain frozen. **Program Analyzer = deterministic structural facts, not coaching/recommendation.** Broader Train-selection migration, additional Programming Engine operations, and block-performance Progress remain deferred.
