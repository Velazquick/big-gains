...
| R6 | **Program portability and fresh-device recovery closure.** Complete the physical production proof for the implemented [Program portability synchronization v1 contract](PROGRAM_PORTABILITY_SYNC_V1.md) so the active Program and approved Routine versions survive device loss/change. | Hosted first-class envelope and hardened gateway live; runtime/cutover implemented; first publication accepted; canonical-order false-conflict fix awaiting guarded deploy and A → B → A reproof | Corrected protected build; physical A → B → A proof; R3; R12 | M | Normative contract and implementation complete; production acceptance paused with capability OFF | Runtime deployment and physical sync proof; no schema/Auth change expected | Cloud recovery can restore training facts while losing the route the user is following. |
