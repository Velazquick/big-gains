...
Implementation update: `v107-active-exercise-units` supersedes the v102 workout-header toggle with per-exercise lb/kg controls. Precedence is exercise override → live profile preference → lb. Load-bearing cards alone show the control; warm-ups, working inputs, prior performance, guidance, and summaries use the same exercise unit. Input converts exactly once (`1 kg = 2.2046226218 lb`); toggles never rewrite canonical loads. Legacy workout-wide metadata migrates onto current exercises lacking valid overrides and is removed. Existing local/offline and active-session JSON persistence carries exercise choices; completion and History normalization strip them. Protected CI, exact-head merge, green-main Pages, and read-only production smoke remain required. No schema, Auth, Program, or production-data mutation.
