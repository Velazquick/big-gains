...
| R6 | **Program portability and fresh-device recovery closure.** Implement the normative [Program portability synchronization v1 contract](PROGRAM_PORTABILITY_SYNC_V1.md) so the active Program and approved Routine versions survive device loss/change. | One first-class per-profile envelope; exact lineage/pins; separately guarded sequence semantics; dependency-ordered transport; explicit legacy cutover | R3; hosted schema/RLS implementation; R12 | XL | Normative portability contract complete; runtime/hosted implementation absent | Supabase, sync, security, migration | Cloud recovery can restore training facts while losing the route the user is following. |
