...
| R6 | **Program portability and fresh-device recovery closure.** Ensure a user's active Program and approved Routine versions survive device loss/change, or provide an equally explicit restore path that meets the trust standard. | `programCapture` is local-only; Program is now a core product object | R3; sync/schema design; R12 | XL | Local contract complete; cloud/portable recovery contract absent | Likely schema, Supabase, RLS, sync, security, migration | Cloud recovery can restore training facts while losing the route the user is following. |
