...
## Picker return-position finding

Repeated blank-workout acceptance reproduced a browser history race: the stable-ID owner issued its instant scroll, then the picker’s same-document `history.back()` restored the previous entry’s scroll position, visibly animating away from the saved set. A local trace recorded scrollBy at 1304 ms followed by popstate at 1306 ms and browser scrolling back to zero. The picker now temporarily owns `history.scrollRestoration` while its entry is open/traversing and restores the caller’s previous setting in the task following popstate. The Train position owner, bookmark format, completion advancement, deliberate-navigation rules and input focus remain unchanged. Tests assert the owned manual policy, its restoration, first-set visibility after traversal, preexisting manual policy preservation, and attach the actual scroll/popstate/bookmark trace.
