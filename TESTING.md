...
The current baseline is 468 passing Chromium tests across 58 files with no expected failures. See [ARCHITECTURE.md](ARCHITECTURE.md) for the runtime boundaries these tests protect and [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) for the required release verification.

Program portability cutover has a separate 42-case Node suite in `tests/program-domain-cutover.test.mjs`. It covers capability-off isolation, meaningful and exact-empty revision-1 publication, remote-absence TOCTOU, durable lost-ACK retry, identical-device convergence, sequence-only conflict classification, explicit remote-empty conflict, deterministic decision snapshots, real cloud/device choices, stale choice/write rejection, exact component revision increments, immutable/lineage/sequence refusal, active/rest/queue guards, History and `programOrigin` preservation, local PE trace retention, unrelated queue preservation, managed/independent profile isolation, and malformed graph rejection. The 43-case recovery suite additionally proves that canonical aggregate/component fingerprints converge when a device retains immutable members in a different array order and that a stored device with no local Program can safely adopt a complete validated remote graph. The runtime rollout suite adds exact versioned capability checks, accepted-transition preservation, provable activation/successor/completion derivation, unprovable-transition refusal, runtime asset/config attachment, and ordinary-UI diagnostic hiding. The existing envelope (46), transport, recovery, and browser suites remain required alongside it.
...
## Dormant Program recovery coverage

`tests/program-domain-recovery.test.mjs` covers the Slice 3 boundary without attaching it to production runtime. It proves capability-off no-op behavior; RLS-row absence versus explicit empty; physical owner and semantic profile isolation; envelope, manifest, pin, component, aggregate, immutable-lineage, and monotonic revision verification; stable no-mutation classification; exact and safe remote advancement; legacy/no-Program distinction; pending Program queue blocking without blocking unrelated domains; initialized-device active/rest deferral; fresh-device Program-before-active-session ordering; exact ID/pin/sequence restoration; frozen predecessor/successor-carry compatibility; no History or `programOrigin` rewrite; no queue ACK/clear; and exact raw rollback after write or readback failure. The module remains absent from `index.html` and `service-worker.js` until hosted migration, cutover, and rollout are separately authorized.
