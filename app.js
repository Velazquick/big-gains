...
bind('activeExercises','click',e=>{
  ...
  if(t.dataset.addSet!==undefined){active.exercises[Number(t.dataset.addSet)].sets.push({id:uid(),weight:'',reps:'',warmup:false,completed:false});autosave();renderActive();return;}
  if(t.dataset.adjust!==undefined){const set=active.exercises[Number(t.dataset.ei)].sets[Number(t.dataset.si)],field=t.dataset.field;set[field]=Math.max(0,(Number(set[field])||0)+Number(t.dataset.adjust));autosave();renderActive();return;}
  if(t.dataset.completeSet){const exerciseIndex=Number(t.dataset.ei),set=active.exercises[exerciseIndex].sets[Number(t.dataset.si)];if(!Number(set.weight)||!Number(set.reps))return;set.completed=!set.completed;autosave();renderActive();if(set.completed){startRestTimer(exerciseIndex);requestAnimationFrame(()=>{if(!active)return;const result=workoutControlsApi.advanceAfterCompletion(active,exerciseIndex);if(!result.advanced)return;autosave();renderActive();if(result.nextIndex>=0)requestAnimationFrame(()=>document.querySelectorAll('#activeExercises .active-exercise')[result.nextIndex]?.scrollIntoView({behavior:'smooth',block:'nearest'}));});}}
});
