...
let workoutTicker=null,deferredPrompt=null,cancelArmedUntil=0,setRemovalArmed=null,setRemovalTimer=null,exerciseRemovalArmed=null,exerciseRemovalTimer=null,exerciseSwapArmed=null,exerciseSwapTimer=null,routineResetArmedUntil=0,routineResetTimer=null;
...
function effectiveExerciseUnit(exercise){return unitsApi.effectiveUnitFor(exercise,state);}
...
function renderLibrary(){renderSelectors();const term=$('exerciseSearch').value,searching=Boolean(exerciseCatalog.normalizeTerm(term)),scoped=!PROFILE.capabilities.allExercises&&selectedDay!=='Other'&&!searching;$('libraryScopeLabel').textContent=scoped?`${displayWorkout(selectedDay)} exercises A–Z`:searching?'All matching exercises':'All exercises A–Z';$('libraryScopeChange').hidden=!scoped;const list=BigGainsExercisePicker.filterExercises({catalog:exerciseCatalog,exercises:searching?CATALOG_EXERCISES:libraryEligibleExercises(),term,muscle:$('muscleFilter').value,equipment:$('equipmentFilter').value});$('exerciseLibrary').innerHTML=list.length?list.map(exercise=>BigGainsExercisePicker.resultMarkup(exercise,{mode:'library',added:Boolean(active?.exercises.some(item=>item.id===exercise.id))})).join(''):'<div class="no-results">No matching exercises.</div>';$('clearLibraryFilters').hidden=!term&&$('muscleFilter').value==='all'&&$('equipmentFilter').value==='all';progressApi.afterLibraryRender();}
...
function openLibraryExercisePicker({fromTrain=false}={}){
  const eligible=new Set(CATALOG_EXERCISES.map(exercise=>exercise.canonicalId));
...
function closestSwapIds(currentExerciseId,excludedExerciseIds=[]){return BigGainsExercisePicker.closestAlternatives({catalog:exerciseCatalog,currentExerciseId,excludedExerciseIds}).map(exercise=>exercise.canonicalId);}
function openActiveExerciseSwap(index){
  const exercise=active?.exercises?.[index];if(!exercise)return false;
  const owner={namespace:ACCOUNT.storageNamespace,workoutId:active.id,exerciseId:exercise.id,index,sets:JSON.stringify(exercise.sets)};
  const sameOwner=()=>owner.namespace===ACCOUNT.storageNamespace&&owner.workoutId===active?.id
    &&active?.exercises?.[owner.index]?.id===owner.exerciseId&&JSON.stringify(active.exercises[owner.index].sets)===owner.sets;
  const excluded=active.exercises.filter((_,itemIndex)=>itemIndex!==index).map(item=>item.id);
  return exercisePicker.open({
    title:`Swap ${exercise.name}`,prompt:'Choose a replacement. New sets start empty. This changes only this workout.',
    currentExerciseId:exercise.id,excludedExerciseIds:[exercise.id,...excluded],
    suggestionIds:closestSwapIds(exercise.id,excluded),suggestionLabel:'Closest alternatives',
    recentExerciseIds:false,browseAllInitially:false,
    returnFocus:()=>{
      const button=document.querySelector(`[data-swap-exercise="${owner.index}"]`);
      return button?.getClientRects().length?button:button?.closest('.active-exercise')?.querySelector('.exercise-toggle');
    },
    onCancel:()=>{if(sameOwner())window.bigGainsTrainPosition?.requestRestore();},
    onSelect:canonicalId=>{
      if(!sameOwner())return;
      const definition=exerciseCatalog.getById(canonicalId);if(!definition)return;
      const result=workoutSessionController.swapExercise(owner.index,definition.id,{confirmed:true});
      if(result.swapped){window.bigGainsTrainPosition?.capture(definition.id);window.bigGainsTrainPosition?.requestRestore();}
    }
  });
}
...
function renderCompletion(workout){
  ...
  $('completionVolume').innerHTML=progressApi.workloadFamiliesMarkup([workout]);
...
bind('libraryScopeChange','click',()=>$('workoutPanel').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'}));
...
bind('activeExercises','input',e=>{const {ei,si,field}=e.target.dataset;if(field&&active&&e.target.tagName==='INPUT'){const value=field==='weight'?(e.target.value===''?'':unitsApi.toCanonicalPounds(e.target.value,effectiveExerciseUnit(active.exercises[Number(ei)]))):e.target.value;if(value!==null)workoutSessionController.updateSet(Number(ei),Number(si),field,value);}const cue=e.target.closest('[data-saved-cue]');if(cue)notesApi.saveCue({activeWorkout:active,state,index:Number(cue.dataset.savedCue),value:cue.value,saveState});const note=e.target.closest('[data-session-note]');if(note){workoutSessionController.focusExercise(Number(note.dataset.sessionNote));notesApi.saveSessionNote({activeWorkout:active,index:Number(note.dataset.sessionNote),value:note.value,autosave});}});
...
bind('activeExercises','click',e=>{
  if(!active)return;
  const unitButton=e.target.closest('[data-exercise-unit]');
  if(unitButton){
    const index=Number(unitButton.dataset.ei),unit=unitButton.dataset.exerciseUnit,exercise=active.exercises[index];
    if(!exercise||!['lb','kg'].includes(unit))return;
    ...
    exerciseSwapArmed=null;clearTimeout(exerciseSwapTimer);
  ...
  const quiet=e.target.closest('.active-exercise.is-collapsed');
  if(quiet&&!e.target.closest('button,input,select,textarea,a,summary')){e.preventDefault();toggleRenderedExercise([...$('activeExercises').children].indexOf(quiet),quiet);return;}
  ...
  if(t.dataset.addSet!==undefined){const index=Number(t.dataset.addSet),set=workoutSessionController.addSet(index);if(set)window.bigGainsTrainPosition?.capture(active.exercises[index].id,set.id);return;}
