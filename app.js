...
let workoutTicker=null,timerTicker=null,timerRemaining=DEFAULT_REST,timerIdleSeconds=DEFAULT_REST,timerNextOverrideSeconds=null,deferredPrompt=null,cancelArmedUntil=0;
...
function resumeRestTimer(){if(!active){hideTimerForInactiveWorkout();return false;}if(!state.restTimerEndsAt){showTimerIdle();if(document.body.dataset.workoutPetState!=='ready')setWorkoutPetState('calm');return true;}if(state.restTimerEndsAt<=Date.now()){completeRestTimer(state.restTimerEndsAt);return true;}runRestTimer();return true;}
let timerFeedbackReset=null,lastAnnouncedCompletionKey=null;
function clearTimerCompletionFeedback(){clearTimeout(timerFeedbackReset);timerFeedbackReset=null;$('timerCard')?.classList.remove('timer-feedback-ready','timer-completing');if($('timerFeedbackStatus'))$('timerFeedbackStatus').textContent='';}
function setTimerLifecycle(next,{message,status='',remaining=timerRemaining}={}){const card=$('timerCard');if(!card)return false;card.classList.remove('hidden','timer-feedback-ready','timer-completing','timer-dismissing');card.dataset.timerState=next;if(next==='ready')card.classList.add('timer-feedback-ready','timer-completing');timerRemaining=Math.max(0,Number(remaining)||0);renderTimer();$('timerNext').textContent=message;$('timerFeedbackStatus').textContent=status;$('timerSkip').disabled=next!=='running';return true;}
function showTimerIdle(message='Timer ready. Complete a set to start rest.'){clearInterval(timerTicker);clearTimerCompletionFeedback();setTimerLifecycle('idle',{message,remaining:timerIdleSeconds});}
function hideTimerForInactiveWorkout(){clearInterval(timerTicker);clearTimerCompletionFeedback();timerIdleSeconds=DEFAULT_REST;timerNextOverrideSeconds=null;setTimerPresetsOpen(false);const card=$('timerCard');if(card){card.dataset.timerState='unavailable';card.classList.add('hidden');}$('timerSkip').disabled=true;}
function showTimerCompletionFeedback(completionKey){if(completionKey===lastAnnouncedCompletionKey)return false;lastAnnouncedCompletionKey=completionKey;clearTimerCompletionFeedback();setTimerLifecycle('ready',{message:"Rest complete. You're up.",status:'Rest complete. Ready for your next set.',remaining:0});timerFeedbackReset=setTimeout(()=>{if(active&&!state.restTimerEndsAt&&$('timerCard')?.dataset.timerState==='ready')showTimerIdle('Ready for your next set.');},3000);return true;}
function setTimerFeedbackStatus(message){const status=$('timerFeedbackStatus');if(status)status.textContent=message;}
function completeRestTimer(deadline){if(!active)return false;const completionKey=`${active.id}:${deadline}`;clearInterval(timerTicker);state.restTimerEndsAt=null;saveState();setWorkoutPetState('ready');showTimerCompletionFeedback(completionKey);workoutTimerFeedback.complete(completionKey);return true;}
function runRestTimer(){clearInterval(timerTicker);clearTimerCompletionFeedback();const deadline=Number(state.restTimerEndsAt);if(!active||!Number.isFinite(deadline)||deadline<=0){if(active)showTimerIdle();else hideTimerForInactiveWorkout();return false;}setTimerLifecycle('running',{message:'Recover. Your next set is waiting.',remaining:Math.max(0,Math.ceil((deadline-Date.now())/1000))});setWorkoutPetState('attentive');let completed=false;const tick=()=>{timerRemaining=Math.max(0,Math.ceil((deadline-Date.now())/1000));renderTimer();if(timerRemaining<=0&&!completed){completed=true;completeRestTimer(deadline);}};tick();if(!completed)timerTicker=setInterval(tick,1000);return true;}
...
bind('timerPresets','click',e=>{const preset=e.target.closest('[data-timer-preset]');if(!preset)return;const seconds=Number(preset.dataset.timerPreset);setTimerPresetsOpen(false);if($('timerCard').dataset.timerState==='running'){state.restTimerEndsAt=Date.now()+seconds*1000;timerIdleSeconds=seconds;saveState();runRestTimer();return;}timerIdleSeconds=seconds;timerNextOverrideSeconds=seconds;showTimerIdle(`Next rest set to ${fmtTime(seconds)}.`);});
bind('timerSkip','click',()=>{if(!active)return;clearInterval(timerTicker);state.restTimerEndsAt=null;saveState();setTimerPresetsOpen(false);showTimerIdle('Rest skipped. Timer ready for the next set.');setWorkoutPetState('calm');});
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState!=='visible'||!active)return;if(state.restTimerEndsAt&&state.restTimerEndsAt<=Date.now())completeRestTimer(state.restTimerEndsAt);else if(state.restTimerEndsAt)runRestTimer();else if($('timerCard').dataset.timerState==='unavailable')showTimerIdle();});
