...
function formatLoggedLoad(exercise,set){const weight=Number(set?.weight)||0;if(exerciseCatalog.loadModeFor(exercise)==='bodyweight')return weight>0?`Bodyweight + ${weight} lb`:'Bodyweight';return `${weight} lb`;}
...
function openHistory(id){
  ...
  const exercises=(w.exercises||[]).map((e,exerciseIndex)=>{
    ...
    const sets=(e.sets||[]).map(s=>{
      ...
      return `<div class="history-set ${isWarmup?'is-warmup':'is-working'}" aria-label="${escapeHtml(`${setLabel}, ${setValue}`)}"><span class="history-set-type">${setLabel}</span><div class="history-set-value"><strong>${escapeHtml(formatLoggedLoad(e,s))}</strong> <span aria-hidden="true">×</span> <strong>${Number(s?.reps)||0}<small> reps</small></strong></div>${best?.id===s.id&&!isWarmup?'<span class="pr-chip">BEST</span>':'<span class="history-set-status"></span>'}</div>`;
    }).join('');
    return `<article class="history-exercise" data-exercise-id="${escapeHtml(exerciseId)}"><div class="history-exercise-head"><div><span class="exercise-order">Exercise ${String(exerciseIndex+1).padStart(2,'0')}</span><h3>${escapeHtml(e.name)}</h3><p>${escapeHtml(e.muscle||'Exercise')} · ${escapeHtml(e.equipment||'Movement')}</p></div><div class="history-exercise-volume"><strong>${formatAnalyticsVolume(exerciseSummary.workingSetVolume)}</strong><span>effective volume</span></div></div><div class="history-set-columns" aria-hidden="true"><span>Type</span><span>Load × reps</span><span></span></div><div class="history-sets">${sets||'<div class="history-sets-empty">No completed sets recorded.</div>'}</div></article>`;
  }).join('');
  const content=$('historyDialogContent');
  content.innerHTML=`${w.entryMethod==='retrospective'?'<div class="history-origin-chip">Entered later</div>':''}<div class="history-summary-grid"><div><span>Duration</span><strong>${fmtTime(summary.durationSeconds)}</strong></div><div><span>Effective volume</span><strong>${formatAnalyticsVolume(summary.workingSetVolume)}</strong></div><div><span>Exercises</span><strong>${summary.exerciseCount}</strong></div><div><span>Working sets</span><strong>${summary.workingSetCount}</strong></div><div><span>PRs</span><strong>${summary.prCount}</strong></div></div>${summary.prCount?`<div class="history-pr-callout"><span class="pr-badge">${summary.prCount} PR${summary.prCount===1?'':'s'}</span><strong>Personal best${summary.prCount===1?'':'s'} earned in this workout.</strong></div>`:''}${w.note?`<div class="history-note history-workout-note"><span>Workout note</span><p>${escapeHtml(w.note)}</p></div>`:''}<div class="history-detail-heading"><span class="label">Exercise breakdown</span><h3>${(w.exercises||[]).length} movement${(w.exercises||[]).length===1?'':'s'}, in workout order</h3></div><div class="history-detail-list">${exercises||'<div class="history-detail-empty">No exercises were recorded for this workout.</div>'}</div>`;
