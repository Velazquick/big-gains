...
const retrospectiveApi=window.bigGainsRetrospective.create({
  ...
  localDateKey,lastPerformance,estimate1RM,workoutLabel:completionWorkoutLabel,loadModeFor:exerciseCatalog.loadModeFor,
...
function analyticsOptions(){return {bodyweight:analyticsApi.profileBodyweight(state.weights),loadModeFor:exerciseCatalog.loadModeFor};}
...
function formatAnalyticsVolume(value){return value===null?'—':`${Math.round(value).toLocaleString('en-US')} lb`;}
function estimate1RM(w,r){return analyticsApi.estimate1RM(w,r);}
function isCompletableSet(exercise,set){return BigGainsWorkoutSessionController.isCompletableSet(exercise,set,exerciseCatalog.loadModeFor(exercise));}
function formatLoggedSet(exercise,set){const reps=Number(set?.reps)||0,weight=Number(set?.weight)||0;if(exerciseCatalog.loadModeFor(exercise)==='bodyweight')return `${weight>0?`Bodyweight + ${weight} lb`:'Bodyweight'} × ${reps}`;return `${weight} lb × ${reps}`;}
...
function renderCompletion(workout){
  ...
  $('completionVolume').textContent=formatAnalyticsVolume(summary.workingSetVolume);
...
function renderCalendarDay(workouts){const [year,month,day]=calendarSelectedKey.split('-').map(Number),date=new Date(year,month-1,day);$('calendarDayHeading').textContent=new Intl.DateTimeFormat('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}).format(date);const box=$('calendarDayWorkouts');if(!workouts.length){box.className='calendar-empty';box.textContent='No training logged';return;}box.className='calendar-day-list';box.innerHTML=workouts.slice().sort((a,b)=>new Date(a.completedAt)-new Date(b.completedAt)).map(workout=>{const summary=analyticsApi.workoutSummary(workout,analyticsOptions());const time=new Intl.DateTimeFormat('en-US',{hour:'numeric',minute:'2-digit'}).format(new Date(workout.completedAt));return `<button type="button" class="calendar-day-workout" data-history-id="${workout.id}"><span><strong>${escapeHtml(completionWorkoutLabel(workout.type))}</strong>${workout.entryMethod==='retrospective'?'<span class="entered-later">Entered later</span>':''}<small>${escapeHtml(time)} · ${fmtTime(summary.durationSeconds)} · ${summary.exerciseCount} exercises · ${summary.workingSetCount} working sets</small><small>${summary.prCount} PR${summary.prCount===1?'':'s'} · View full workout →</small></span><span class="calendar-workout-volume"><strong>${formatAnalyticsVolume(summary.workingSetVolume)}</strong><small>volume</small></span></button>`;}).join('');}
function openHistory(id){const w=state.workouts.find(x=>x.id===id);if(!w)return;const title=completionWorkoutLabel(w.type),summary=analyticsApi.workoutSummary(w,analyticsOptions());$('historyDialogTitle').textContent=title;$('historyDialogDate').textContent=fmtDateLong(w.completedAt);const totalSets=(w.exercises||[]).flatMap(e=>e.sets||[]).length;const exercises=(w.exercises||[]).map(e=>{const best=analyticsApi.bestWorkingSet(e,analyticsOptions()),exerciseId=e.definitionId||e.id;const sets=(e.sets||[]).map((s,i)=>`<div class="history-set"><span class="${s.warmup?'warmup-chip':''}">${s.warmup?'Warm-up':`Set ${i+1}`}</span><strong>${escapeHtml(formatLoggedSet(e,s))}</strong>${best?.id===s.id&&!s.warmup?'<span class="pr-chip">BEST</span>':'<span></span>'}</div>`).join('');return `<article class="history-exercise" data-exercise-id="${escapeHtml(exerciseId)}"><div class="history-exercise-head"><div><span class="exercise-muscle">${escapeHtml(e.muscle||'Exercise')}</span><h3>${escapeHtml(e.name)}</h3><small>${escapeHtml(e.equipment||'')} · ${formatAnalyticsVolume(volumeForExercise(e))} volume</small></div></div><div class="history-sets">${sets}</div></article>`;}).join('');const content=$('historyDialogContent');content.innerHTML=`${w.entryMethod==='retrospective'?'<p class="entered-later">Entered later</p>':''}${w.note?`<div class="history-note"><span>Workout note</span><p>${escapeHtml(w.note)}</p></div>`:''}<div class="history-summary-grid"><div><span>Duration</span><strong>${fmtTime(summary.durationSeconds)}</strong></div><div><span>Total volume</span><strong>${formatAnalyticsVolume(summary.workingSetVolume)}</strong></div><div><span>Work completed</span><strong>${(w.exercises||[]).length} exercises · ${totalSets} sets</strong></div></div>${summary.prCount?`<div class="previous-note">${summary.prCount} personal record${summary.prCount===1?'':'s'} earned in this workout.</div>`:''}<div class="history-detail-list">${exercises}</div>`;notesApi.renderHistoryNotes({workout:w,container:content,escapeHtml});progressApi.afterHistoryOpen();const dialog=$('historyDialog');dialog.dataset.workoutId=w.id;$('historyWorkoutActions').hidden=false;$('deleteWorkoutConfirmation').hidden=true;$('deleteWorkoutError').textContent='';$('confirmDeleteCompletedWorkout').disabled=false;if(dialog.showModal)dialog.showModal();else dialog.setAttribute('open','');}
...
progressApi.initialize({getState:()=>state,getAnalyticsOptions:analyticsOptions,exercises:CATALOG_EXERCISES,analytics:analyticsApi,fmtDate,escapeHtml,workoutLabel:completionWorkoutLabel,closeHistory,closeRoutineEditor});
