-- Phase 4G independent-account onboarding and allowlisted presentation.
-- Authorization continues to depend only on account ownership. Presentation
-- columns are render-only values and are never referenced by an RLS policy.

alter table public.profiles
  add column pet_enabled boolean not null default true,
  add column accent text not null default 'ember',
  add column theme text not null default 'performance-dark';

alter table public.profiles
  add constraint profiles_accent_check check (accent in ('ember', 'rose', 'cobalt')),
  add constraint profiles_theme_check check (theme in ('performance-dark', 'wellness-light')),
  add constraint profiles_display_name_length_check check (length(trim(display_name)) between 1 and 60),
  add constraint profiles_display_name_controls_check check (display_name !~ '[[:cntrl:]]');

alter table public.accounts
  add constraint accounts_display_name_length_check check (length(trim(display_name)) between 1 and 60),
  add constraint accounts_display_name_controls_check check (display_name !~ '[[:cntrl:]]');

-- Preserve the deployed managed presentation exactly. The defaults above map
-- to Jorge; Alexa keeps her existing wellness palette.
update public.profiles
set accent = 'rose', theme = 'wellness-light'
where client_id = 'alexa';

comment on column public.profiles.pet_enabled is
  'Render-only allowlisted presentation. Never use for ownership or authorization.';
comment on column public.profiles.accent is
  'Render-only token name: ember, rose, or cobalt.';
comment on column public.profiles.theme is
  'Render-only token name: performance-dark or wellness-light.';

-- Authenticated callers retain INSERT privilege because the security-invoker
-- bootstrap needs it, but direct Data API inserts fail unless the RPC has set
-- this transaction-local, caller-bound guard.
...
drop policy profiles_insert_owned on public.profiles;
