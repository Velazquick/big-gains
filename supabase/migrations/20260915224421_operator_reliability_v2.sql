...
begin
 ...
 if section in ('overview','traffic','funnel') then
   result:=result||jsonb_build_object('traffic',coalesce((select jsonb_agg(to_jsonb(t)) from (
    ...
    'visitor_windows',(select jsonb_build_object('today',count(*) filter(where received_at>=date_trunc('day',now() at time zone 'UTC') at time zone 'UTC'),'days7',count(*) filter(where received_at>=now()-interval '7 days'),'days30',count(*)) from private.visitor_events where event_name='visit' and received_at>=now()-interval '30 days' and (source_filter is null or source=source_filter)),
    'visitor_environments',coalesce((select jsonb_agg(to_jsonb(t)) from (select platform,browser,mode,count(*) as sessions from private.visitor_events where event_name='visit' and received_at>=since_time and (source_filter is null or source=source_filter) group by platform,browser,mode)t),'[]'));
 end if;
